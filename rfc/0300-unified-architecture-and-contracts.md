...
本文件是 AuroraBot 唯一的设计基准。当前工作树只描述现行实现，不保留旧架构的兼容层、迁移代码或并行公共契约。
...
AuroraBot 当前是一个用于发现自主智能体基本结构的实验性框架，不是待发布产品。现阶段只实现并验证一个完整的最小循环，
不以部署便利、故障恢复、历史数据兼容、运维面板或第三方扩展生态为目标。
...
- **先求可解释，再求可靠**：只有最小语义稳定后，才重新引入持久化、恢复、并发、授权和运维设施。
- **拒绝预支抽象**：没有两个真实实现或明确不变量时，不建立注册表、生命周期体系或通用扩展接口。
...
首版不做自动裁剪和摘要；超过显式字符上界时立即失败，让上下文策略保持可见。记忆日后只能作为产生 message 或 system
片段的上游能力重新加入，不能隐藏在组装器内部。
...
核心值对象可直接由 Python 构造。项目组合根读取一份最小 TOML，包含 system 片段、root profile/model/tools 和运行上界；
密钥只来自环境变量。

当前不定义运行时数据库、schema 版本、迁移、会话归档、费用库、记忆库或面板存储。需要实验持久化时，先以完整
